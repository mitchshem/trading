...
class CustomReplayEngine(ReplayEngine):
    ...
    def process_candle(self, db_session, candle: Dict) -> Optional[Dict]:
        ...
        # Process pending orders at candle open
        open_time = current_candle.get("open_time", current_candle["time"])
        executed_trades = self.broker.process_pending_orders(
            current_open_price=current_candle["open"],
            timestamp=open_time
        )
        ...
        kill_switch_orders = self.broker.check_and_enforce_risk_controls(
            ...
            timestamp=current_candle.get("close_time", current_candle["time"])
        )
        ...
        if result["signal"] == "BUY" and current_atr is not None and self.symbol not in self.stop_loss_symbols and buy_allowed:
            # Calculate stop distance with custom multiplier
            stop_distance = self.stop_loss_atr_multiplier * current_atr
